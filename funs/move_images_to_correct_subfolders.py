...
def main():
	...
	try:
		...
		cursed_image_list= glob.glob(os.path.expanduser('data/reddit_sub_cursedimages/*')
			)+glob.glob(os.path.expanduser('data/twitter_account_cursedimages/*'))
		neutral_image_list  = glob.glob(os.path.expanduser('data/neutral/*'))

		# Going with a 70/20/remainder up to 10% train/val/test/ split
		random.seed(324823)
		all_images_list = [cursed_image_list,neutral_image_list]
		for each_list_idx,each_list in tqdm(enumerate(all_images_list)):
			...
			#Destination paths
			train_path = os.path.expanduser('data/train/')+image_type+'/'
			val_path   = os.path.expanduser('data/validation/')+image_type+'/'
			test_path  = os.path.expanduser('ata/test/')+image_type+'/'
